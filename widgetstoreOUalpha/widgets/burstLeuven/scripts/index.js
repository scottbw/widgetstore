$(document).ready(function() {
    ...
    //var WS_DOMAIN="http://yahoo.com/";
    var WS_DOMAIN="http://localhost/";
    ...
    function updatePublicationDetails(pid){
        ...
        $.ajax({
            ...
            success: function(m){
                ...
                pid=m["rdf:RDF"]["burst:publication"]["rdf:ID"];
                ptitle=m["rdf:RDF"]["burst:publication"]["swrc:title"];
                pabstract=m["rdf:RDF"]["burst:publication"]["swrc:abstract"];
                pwebLink=m["rdf:RDF"]["burst:publication"]["swrc:link"];
                pdescription=m["rdf:RDF"]["burst:publication"]["swrc:description"];
                planguage=m["rdf:RDF"]["burst:publication"]["xml:lang"];
                pdate=m["rdf:RDF"]["burst:publication"]["dc:date"];
                pbooktitle=m["rdf:RDF"]["burst:publication"]["swrc:booktitle"];
                pkeywords=m["rdf:RDF"]["burst:publication"]["swrc:keywords"];
                pconference=m["rdf:RDF"]["burst:publication"]["swrc:conference"];
                htmlContent="";
                htmlContent+="<p>Title:"+ptitle+"</p>";
                if (pabstract!=undefined) htmlContent="<p>Abstract:"+pabstract+"</p>";
                htmlContent+="<p>Authors:";
                authors=m["rdf:RDF"]["burst:publication"]["swrc:author"];
                htmlContent+="<ul class=\"authorsList\">";
                for (i=0;i<authors.length;i++) {
                    ////console.log(a+"  =  "+authors[a]);
                    htmlContent+="<li><p> <em>  "+authors[i]["swrc:Person"]["swrc:name"]+"</em>";
                    if (authors[i]["swrc:Person"]["swrc:affiliation"]!=undefined) htmlContent+=" affiliated to "+authors[i]["swrc:Person"]["swrc:affiliation"]+"";
                ...
                if (pdate!=undefined) htmlContent+="<p>Date:"+pdate+"</p>";
                ...
                $('#pubDetails ul.authorsList li i').click(function(){
                    switchTabs(2);
                    insertToSelectedAuthors2($(this).parent().parent().find('span').text(), $(this).parent().parent().find('em').text());
                });
    ...
    function updatePublicationHits (m){
        ...
        if (hits==0)
            ...
            for (i=0;i<m["rdf:RDF"]["burst:publication"].length;i++){
                pid=m["rdf:RDF"]["burst:publication"][i]["rdf:ID"];
                ptitle=m["rdf:RDF"]["burst:publication"][i]["swrc:title"];
                //pabstract=m["burst:publication"][i]["swrc:abstract"];
                pwebLink=m["rdf:RDF"]["burst:publication"][i]["swrc:link"];
                pdescription=m["rdf:RDF"]["burst:publication"][i]["swrc:description"];
                htmlContent+="<li>";
                ...
                htmlContent+="    "+ptitle+" <em>view details</em>&nbsp;&nbsp;<a href='http://universe.stellarnet.eu/burst/hitcount.php?id="+pid+"'target='_blank'>Download</a>";
                htmlContent+="    <span style='display:none'>"+pdescription+"</span>";
                htmlContent+="</li>";
            };
            htmlContent+="</ul>";
        }
    ...
    function insertToSelectedAuthors(xmlFormattedAuthor){
        ...
        $('#selectedAuthors ul').append("<li><input type='checkbox' class='checkbox' name='result[]' value='"+xmlFormattedAuthor.id+"' checked />"+xmlFormattedAuthor.value+"</li>");
    ...
    $('#searchSelectedAuthorsInput').click(function(){
        ...
        $('#selectedAuthors ul li').each(function(){
            if ($(this).find('.checkbox').is(':checked')) {
                selAuthorsArray.push($(this).find('input').val());
            }
        });
