...
function IWCinit(format, initSharedData) {
	...
	if (IWCstatusMessages) setStatusFade('IWC configured ...');
}
...
function IWCsetVarJSON(ns, name, value) {
	if (IWCstatusMessages) setStatusFade('sending data ...');
...
function IWCdelVarJSON(name) {
	if (IWCstatusMessages) setStatusFade('deleting data ...');
        Widget.sharedDataForKey(
                IWCnameSpace,
		function(data) {
			...
			if (data != "No matching key found" && data != "") {
				...
				for (var name in obj) {
					...
					if (IWCdata[name] != obj[name]) {
						if (IWCstatusMessages) setStatusFade('retrieving data ...');
